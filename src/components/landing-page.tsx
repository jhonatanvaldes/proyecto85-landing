...
const reportPages = [
  {
    ...
    eyebrow: "Vista protegida",
    summary: "Una lectura inicial del Índice Proyecto 85, tus fortalezas y tus prioridades.",
    points: ["Índice P85", "Interpretación clínica", "Prioridades"]
  },
  {
    title: "7 pilares",
    eyebrow: "Mapa preventivo",
    summary: "Una representación clara de las áreas que más influyen en tu salud futura.",
    points: ["Corazón", "Fuerza", "Mente"]
  },
  {
    title: "Fortalezas y prioridades",
    eyebrow: "Lo que más importa",
    summary: "Distingue lo que ya trabaja a tu favor y donde existe mayor oportunidad.",
    points: ["Fortalezas", "Áreas de oportunidad", "Prioridad clínica"]
  },
  {
    title: "Salud futura",
    eyebrow: "Próximas décadas",
    summary: "Integra señales preventivas para entender riesgos y oportunidades de forma práctica.",
    points: ["Riesgo dominante", "Independencia", "Función"]
  },
  {
    title: "Orientación médica",
    eyebrow: "Orientación",
    summary: "Recomendaciones personalizadas para tomar mejores decisiones desde hoy.",
    points: ["Objetivos", "Acciones", "Seguimiento médico"]
  },
  {
    title: "Árbol de longevidad",
    eyebrow: "Síntesis visual",
    summary: "Una síntesis elegante de hábitos, oportunidades y factores protectores.",
    points: ["Hábitos", "Reserva de salud", "Calidad de vida"]
  }
];
...
function ReportMiniPage({ page, active }: { page: (typeof reportPages)[number]; active: boolean }) {
  return (
    <div
      className={cn(
        "relative h-[500px] min-w-[355px] overflow-hidden rounded-lg border border-gold-500/25 bg-ivory shadow-premium transition-all duration-500 sm:min-w-[405px]",
        active ? "scale-100 opacity-100" : "scale-[0.94] opacity-55"
      )}
    >
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_8%,rgba(216,175,79,0.22),transparent_30%)]" />
      <div className="absolute left-0 right-0 top-0 flex h-14 items-center justify-between bg-navy-950 px-7 text-xs font-bold uppercase tracking-[0.22em] text-gold-300">
        <span>Proyecto 85</span>
        <span>Vista protegida</span>
      </div>
      <div className="relative flex h-full flex-col px-8 pb-8 pt-24">
        <div className="mb-6 flex items-center justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.28em] text-gold-600">{page.eyebrow}</p>
            <h3 className="serif-title mt-2 text-4xl font-semibold leading-none text-navy-950">{page.title}</h3>
          </div>
          <div className="grid h-12 w-12 place-items-center rounded-full border border-gold-500/45 bg-white text-gold-600 shadow-gold">
            <LockKeyhole className="h-5 w-5" />
          </div>
        </div>
        <p className="text-sm leading-7 text-graphite">{page.summary}</p>
        <div className="mt-8 space-y-4">
          {page.points.map((point, index) => (
            <div key={point} className="rounded-md border border-gold-500/20 bg-white/70 p-4">
              <div className="flex items-center justify-between gap-5">
                <span className="text-[11px] font-bold uppercase tracking-[0.18em] text-navy-950">{point}</span>
                <span className="serif-title text-3xl text-gold-600">{index === 0 ? "85" : index === 1 ? "7" : "01"}</span>
              </div>
              <div className="mt-3 h-2 rounded-full bg-[#E7DED0]">
                <div
                  className="h-full rounded-full bg-gradient-to-r from-gold-500 via-gold-300 to-gold-600"
        ...
        <div className="mt-auto rounded-md border border-navy-900/10 bg-navy-950 px-5 py-4 text-center">
          <p className="text-[10px] font-bold uppercase tracking-[0.24em] text-gold-300">
            Contenido personalizado entregado solo al paciente
          </p>
        </div>
      </div>
    </div>
...
export function LandingPage() {
  ...
  const whatsappHref = useMemo(
    () =>
      "https://wa.me/524731013302?text=Hola%2C%20quiero%20agendar%20mi%20evaluaci%C3%B3n%20Proyecto%2085.",
    []
  );
  ...
  return (
    <main className="overflow-hidden bg-ivory text-navy-950">
      ...
      <section id="inicio" className="relative min-h-screen bg-navy-950 pt-24 text-white">
        ...
        <div className="absolute inset-0 bg-[linear-gradient(115deg,rgba(2,11,29,0.98)_0%,rgba(2,11,29,0.92)_42%,rgba(2,11,29,0.42)_72%,rgba(2,11,29,0.82)_100%)]" />
        ...
        <ElectroLine />
        <div className="container relative grid min-h-[calc(100vh-6rem)] items-center gap-12 pb-16 lg:grid-cols-[0.9fr_1.1fr]">
          <FadeIn className="max-w-3xl">
            ...
            <p className="mt-5 inline-flex rounded-full border border-gold-500/25 bg-gold-500/10 px-4 py-2 text-sm font-semibold uppercase tracking-[0.18em] text-gold-300">
              Conoce tu presente, construye tu futuro.
            </p>
          ...
          <FadeIn delay={0.15} className="relative">
            <div className="premium-border relative z-20 ml-auto max-w-[660px] overflow-hidden rounded-lg bg-white/[0.06] p-3 shadow-premium backdrop-blur-sm">
              <div className="relative min-h-[520px] overflow-hidden rounded-md bg-navy-950">
                <Image
                  src="/images/brand/publicidad-adultos-activos.png"
                  alt="Adultos activos representando longevidad, fuerza e independencia"
                  fill
                  priority
                ...
                <div className="absolute left-6 top-6 rounded-full border border-white/15 bg-navy-950/75 px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-white/80 backdrop-blur">
                  Longevidad activa
                </div>
                <div className="absolute bottom-6 left-6 right-6 rounded-md border border-gold-500/25 bg-navy-950/76 p-5 backdrop-blur">
                  <div className="flex items-center gap-3">
                    <span className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-gold-500 text-navy-950">
                      <Play className="h-5 w-5 fill-current" />
                    </span>
                    <div>
                      <p className="text-sm font-bold uppercase tracking-[0.18em] text-gold-300">Experiencia Proyecto 85</p>
                      <p className="mt-1 text-sm text-white/76">Fuerza, independencia, energía y calidad de vida.</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="absolute -bottom-8 -left-4 z-30 hidden w-48 rotate-[-4deg] rounded-lg border border-gold-500/25 bg-navy-950/92 p-2 shadow-gold backdrop-blur md:block">
              <Image
                src="/images/report/cover.png"
                alt="Portada del reporte Proyecto 85"
                width={970}
                height={1254}
      ...
      <section className="relative overflow-hidden bg-[#F1EADF] py-16 md:py-24">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_20%,rgba(216,175,79,0.22),transparent_30%)]" />
        <div className="container relative grid gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          <FadeIn>
            <SectionLabel>Medicina privada de longevidad</SectionLabel>
            <h2 className="serif-title text-4xl font-semibold leading-tight text-navy-950 md:text-6xl">
              Inversión en tu salud hoy, calidad de vida mañana.
            </h2>
            <p className="mt-6 text-lg leading-8 text-graphite">
              La evaluación está diseñada para darte claridad médica: qué está funcionando a tu favor,
              qué conviene atender primero y cómo proteger independencia, energía y funcionalidad.
            </p>
          </FadeIn>
          <FadeIn delay={0.1}>
            <div className="grid gap-4 sm:grid-cols-3">
              {[
                ["85", "Índice Proyecto 85"],
                ["7", "Pilares evaluados"],
                ["1", "Reporte exclusivo"]
              ].map(([value, label]) => (
                <div key={label} className="rounded-lg border border-gold-500/25 bg-ivory p-6 shadow-gold">
                  <p className="serif-title gold-text text-6xl font-semibold leading-none">{value}</p>
                  <p className="mt-4 text-xs font-bold uppercase tracking-[0.2em] text-navy-950">{label}</p>
                </div>
              ))}
            </div>
          </FadeIn>
          ...
          <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:items-end">
            <FadeIn>
              ...
              <p className="mt-6 text-lg leading-8 text-white/68">
                Mostramos la portada y la estructura general, pero no exponemos páginas internas.
                El valor está en la interpretación personalizada que recibe cada paciente.
              </p>
              <div className="mt-6 inline-flex items-center gap-3 rounded-full border border-gold-500/25 bg-gold-500/10 px-4 py-2 text-sm font-semibold text-gold-300">
                <ShieldCheck className="h-4 w-4" />
                Contenido protegido, entrega privada
              </div>
            </FadeIn>
      ...
      <section id="precio" className="relative bg-navy-950 py-20 text-white md:py-28">
        ...
        <FadeIn className="container relative mx-auto max-w-4xl text-center">
          ...
          <div className="premium-border rounded-lg bg-white/[0.06] p-8 backdrop-blur md:p-12">
            ...
            <p className="mx-auto mt-3 max-w-xl text-sm font-semibold uppercase tracking-[0.18em] text-gold-300">
              Inversión en tu salud hoy, es calidad de vida mañana.
            </p>
